...
DIRECTION_MAP = {
    pygame.K_UP: Dir.U, pygame.K_DOWN: Dir.D, pygame.K_LEFT: Dir.L, pygame.K_RIGHT: Dir.R
}

Pt = namedtuple('Pt', 'x, y')
SNAKE_BLOCK, SNAKE_SPEED = 20, 40
WHITE, GREEN, BLACK, GRAY = (255, 255, 255), (0, 255, 0), (0, 0, 0), (128,128,128)


class SnakeGameAI:
    # Set the dimensions of the screen to be larger than the playing field by default
    #apple_pos, frame_iteration, score, snake, head, paused, direction = (None,) * 7

    def __init__(self, w=800, h=600, td_width=200):
        self.w, self.h, self.text_display_width = w, h, td_width
        self.playing_field_w, self.playing_field_h = w - td_width if w - td_width > 0 else 0, h
        # Set up the display
        self.screen = pygame.display.set_mode((w, h))
        pygame.display.set_caption("Snake Game")
        # Create a surface for the playing field, and create a surface for the text display
        self.playing_field, self.text_display = pygame.Surface(
            (self.playing_field_w, self.playing_field_h)), pygame.Surface((td_width, h))
        # Load the images
        self._load_images()
    ...
    def reset(self):
        ...
        self.head = Pt(self.w / 2, self.h / 2)
        self.snake = [self.head, Pt(self.head.x - SNAKE_BLOCK, self.head.y),
                      Pt(self.head.x - (2 * SNAKE_BLOCK), self.head.y)]
        # Initialize the score and frame iteration counter
        self.score, self.frame_iteration = 0, 0
        self.apple_pos = Pt(
            *self.generate_new_apple_pos(self.snake, self.playing_field_w, self.playing_field_h, SNAKE_BLOCK))
    ...
    def is_apple_overlapping(apple_position, snake):
        ...
        return any(block == apple_position for block in snake)
    ...
    def play_step(self, action):
        ...
        if self.head == self.apple_pos:
            ...
            apple_pos_x, apple_pos_y = self.generate_new_apple_pos(self.snake,
                                                                   ...
                                                                   SNAKE_BLOCK)
            self.apple_pos = Pt(apple_pos_x, apple_pos_y)
        else:
            self.snake.pop()
    ...
    def _move(self, action):
        ...
        if change_to == Dir.U and self.direction != Dir.D:
            self.direction = Dir.U
        elif change_to == Dir.D and self.direction != Dir.U:
            self.direction = Dir.D
        elif change_to == Dir.L and self.direction != Dir.R:
            self.direction = Dir.L
        elif change_to == Dir.R and self.direction != Dir.L:
            self.direction = Dir.R
        ...
        if self.direction == Dir.R:
            x += SNAKE_BLOCK
        elif self.direction == Dir.L:
            x -= SNAKE_BLOCK
        elif self.direction == Dir.D:
            y += SNAKE_BLOCK
        elif self.direction == Dir.U:
            y -= SNAKE_BLOCK
    ...
    def _update_ui(self):
        ...
        self.playing_field.fill(WHITE)
        for i, block in enumerate(self.snake):
            # Draw the head
            if i == 0:
                if self.direction == Dir.U:
